...
class BrevetDB:
    ...
    def basic_information_liste(self):
        infos = self.cursor.execute('SELECT prenom_s, nom FROM Candidat').fetchall()
        self.conn.commit()
        return infos
    ...
    def choix_epreuve_facutative(self, num_table):
    ...
    def ajouter_candidat(self, num, prenoms, nom, date_naissance, lieu_naissance, sexe, nationalite, etablissement,
        ...
        self.cursor.execute("""
                        INSERT INTO Candidat (
                            num_table, prenom_s, nom, date_naissance, lieu_naissance, sexe,
                            nationalite, etablissement, choix_epreuve_facultative, epreuve_facultative, 
                            aptitude_Sportive, type_candidat,id_livret, anonymat
                        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """, (
            num, prenoms, nom, date_naissance, lieu_naissance, sexe, nationalite, etablissement,
            "OUI" if epreuve_facultative == "NEUTRE" else "NON", epreuve_facultative,
    ...
    def fill_db(self, xslx_file_path):
        ...
        self.cursor.execute("""CREATE TABLE Jury(id_jury INTEGER,ia TEXT NOT NULL,ief TEXT NOT NULL,
                   localite TEXT NOT NULL,centre_examen TEXT NOT NULL,president_jury TEXT NOT NULL,telephone TEXT NOT NULL,
                   PRIMARY KEY(id_jury));""")
